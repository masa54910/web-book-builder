...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { data: purchases, error } = await admin.from("book_purchases").select("id,book_id,access_code_hash,payment_status,revoked_at,stripe_livemode").eq("book_id", book.id).eq("stripe_livemode", expectedLivemode).eq("payment_status", "paid").is("revoked_at", null).limit(20);
