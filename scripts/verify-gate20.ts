...
const contactNotification = read("src/lib/server/contactNotification.ts");
...
assert.match(contactRoute, /Notification is best-effort/);
assert.match(contactNotification, /CONTACT_NOTIFICATION_EMAIL/);
assert.match(contactNotification, /RESEND_API_KEY/);
assert.match(contactNotification, /RESEND_FROM_EMAIL/);
assert.match(contactNotification, /api\.resend\.com\/emails/);
assert.match(contactNotification, /reply_to/);
assert.match(contactNotification, /text:/);
assert.doesNotMatch(contactNotification, /NEXT_PUBLIC_CONTACT_NOTIFICATION_EMAIL/);
assert.match(envExample, /CONTACT_NOTIFICATION_EMAIL=/);
assert.match(envExample, /RESEND_API_KEY=/);
assert.match(envExample, /RESEND_FROM_EMAIL=/);
assert.doesNotMatch(contactRoute, /CONTACT_NOTIFICATION_EMAIL\s*=/);
