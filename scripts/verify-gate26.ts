...
assert(plan.includes("operation_standard") && plan.includes("publicationSlots: 1") && plan.includes("publicationSlots: 10"), "final plan definitions remain authoritative");
assert(repository.includes("hasOperationSalesEntitlement") && repository.includes('eq("status", "active")'), "operation sales entitlement gate is server-side");
assert(sales.includes("hasOperationSalesEntitlement") && sales.includes("作品販売には運用プランが必要です"), "new Connect sales require an operation plan");
...
assert(read("supabase/migrations/019_gate20_pricing_plan_alignment.sql").includes("operation_standard"), "Standard plan migration is additive");
